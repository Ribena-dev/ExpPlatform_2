...
class LidarProcessor(object):
    ...
    flag_bl = 0
    flag_br = 0
    flag_old = [flag_f,flag_l,flag_r,flag_fl,flag_fr,flag_bl,flag_br]
    flag_new = [flag_f,flag_l,flag_r,flag_fl,flag_fr,flag_bl,flag_br]
    ...
    dist_bl = 0
    dist_br = 0
    dist = [dist_l,dist_fl,dist_f,dist_fr,dist_r,dist_bl,dist_br]
    ...
    def update_distance(self):
        ...
        self.dist_bl = self.sonar_object.sonar_min[0]
        self.dist_br = self.sonar_object.sonar_min[1]
        ...
        self.dist = [self.dist_l, self.dist_fl, self.dist_f, self.dist_fr, self.dist_r,self.dist_bl,self.dist_br]

        self.save_data((self.dist_f))

        self.update_flags(self.dist_f,self.dist_l,self.dist_r,self.dist_fl,self.dist_fr,self.dist_bl,self.dist_br)

    def update_speed(self):
        if self.flag_old != self.flag_new:
            if self.flag_f == 3 | self.flag_fl == 3 | self.flag_fr == 3 | self.flag_bl == 3 | self.flag_br == 3:
                print("force stop")
                twist = Twist() # force stop if needed
                self.pub.publish(twist)
        pass

    def update_flags(self,dist_f,dist_l,dist_r,dist_fl,dist_fr,dist_bl,dist_br):
        self.flag_old = [self.flag_f, self.flag_l, self.flag_r, self.flag_fl, self.flag_fr,self.dist_bl,self.flag_br]
