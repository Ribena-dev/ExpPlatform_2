# this is a modified Experiment_based.py, inded for the smaller room in md2 with 4 poster locations at each corner
...
# publisher_e = rospy.Publisher('emergency_msgs', Int16, queue_size=2)
...
if use_eyelink:
    ...
    tracker.send_message('Cue Offset {}'.format("00"))
    tracker.send_message('Reward {}'.format("00"))
    tracker.send_message('Timeout {}'.format("00"))
    tracker.send_message('End Trial {}'.format("00"))
...
status = "Normal"
phase = "Pre Trial Interval"
encoding_dict = {"Start Trial": 10, "Cue Offset": 20, "Reward": 30, "Timeout": 40}
pt = PointStamped()
...
while True:
    ...
    if phase == "Start Trial":
        if mini_timer.getTime() > cue_duration:
            ...
            if use_eyelink:
                tracker.send_message('Cue Offset {}'.format(message.data))
    ...
    if phase == "Cue Offset":
        if mini_timer.getTime() > move_duration:
            ...
            if use_eyelink:
                tracker.send_message('Timeout {}'.format(message.data))
    ...
    if phase == "Reward":
        ...
        if mini_timer.getTime() > reward_duration:
            ...
            if curr_trial == num_trials - 1:
                ...
                if use_eyelink:
                    tracker.send_message('End Trial {}'.format(message.data))
    ...
    if (zone_entered > 0 and master_timer.getTime() - zone_entered > dest_duration) or (input == 32 and phase == "Cue Offset"):        
        phase = "Reward"
        zone_entered = -1
        message = Int16()
        message.data = encoding_dict[phase]+targets[curr_target]+1
