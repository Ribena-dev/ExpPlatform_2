...
# to add: a moving average from  joytick values
	# reduce sudden  variations in movement 
...
class JoystickProcessor(object):
    ...
    def callback(self, data):
        ...
        if(override == False):
            ...
            smoothed_speed = sum(self.speed_buffer)/5
            smoothed_angular_speed = sum(self.angular_speed_buffer)/5
    ...
    def move(self,smooth_speed,smooth_angular_speed, data):
        ...
        if toggle_check == -1.0: #Toggle to disable obstacle lock
            ...
            if (angular_speed > 0.8) & (smooth_speed > -1): #0.2
