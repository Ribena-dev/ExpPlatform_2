#!/usr/bin/env python
#this uses the original combined joystick.py to include a secondary lidar, made sperate for testing and dev purposes
...
class LaserSubs(object):
    ...
    def init_laser_range(self):
        ...
        for i in range(3):
            ...
            if self.front_laser_ranges is None:
                try:
...
class JoystickProcessor(object):
    ...
    def move(self, data):
        ...
        if toggle_check == -1.0: #Toggle to disable obstacle lock
            ...
            if speed > 0.6 and np.abs(data.axes[0]) < 0.1:  # Joystick is indicating to move forward, 0.2
        ...
        if toggle_check == -1.0: #Toggle to disable obstacle lock
            ...
            if angular_speed > 0.6 and data.axes[1] < 0.1: #0.2
